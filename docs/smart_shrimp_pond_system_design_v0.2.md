...
## Member B — Data / ML / Agent / Application
...
# 37. Shared Repository Structure
...
# 40. Two-Month Roadmap

## Week 1 — Domain / Architecture
...
実池の完全自律制御はMVPの必須条件とせず、安全性・設備条件に応じて段階的に検討する。
...
## Phase 6 — Autonomous Aquaculture
...
## Phase 7 — Multi-Pond Optimization
...
# 45. Demonstration Scenario

## Scenario: DO Deterioration
...
### Step 4 — Agent Context Retrieval
...
```text
Current DO is still within
the acceptable range.

However, the trend suggests
future deterioration.
```

### Step 6 — Predictive Alert

```text
Predictive Warning

High risk of undesirable DO
condition within 3 hours.

Recommended Action:
Check pump / aeration system.
```
...
# 47. Three-Day Proposal Preparation
...
# 50. Design Status Management

この設計書はLiving Documentとして管理する。
...
# Appendix A — One-line System Architecture

```text
IoT + Environment + Feed + Farm Operation
→ Data
→ ML
→ Predictive Alert
→ Agent
...
# Appendix B — One-line Development Architecture

```text
Human Goal → Agentic SDLC → Design → Build → Verify → Human Review → Iterate
```

# Appendix C — One-line Project Vision

```text
Monitor → Understand → Predict → Notify → Recommend → Act → Learn
```

# Appendix D — Critical Research Question

> **「6種類のIoTセンサーと外部環境・餌・養殖管理データを統合したとき、何を予測すれば『エビにとって望ましくない環境になる前に通知・対応できる』のか？」**
